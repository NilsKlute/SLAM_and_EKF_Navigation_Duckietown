        ...
        if good_seg_count < self.min_segs.value:
            ...
            if self.verbose.value >= 1:
                rospy.loginfo_throttle(0.5,
                    f"[StopLine] not detected  "
                    f"red={total_red} behind={behind_count} off_lane={off_lane_count} "
                    f"good={int(good_seg_count)}/{self.min_segs.value}"
                )
        else:
            ...
            dist_ahead = -stop_pose.x  # positive = stop line is ahead of us
            stop_line_reading_msg.at_stop_line = dist_ahead < self.stop_distance.value
            ...
            if self.verbose.value >= 1:
                tag = "*** AT STOP LINE *** " if stop_line_reading_msg.at_stop_line else ""
                log_fn = rospy.logwarn_throttle if stop_line_reading_msg.at_stop_line else rospy.loginfo_throttle
                log_fn(0.5,
                    f"[StopLine] {tag}"
                    f"red={total_red} behind={behind_count} off_lane={off_lane_count} "
                    f"good={int(good_seg_count)}/{self.min_segs.value}  "
                    f"dist={dist_ahead:.3f}m (trigger@{self.stop_distance.value:.2f}m)  "
                    f"lat={stop_pose.y:+.3f}m"
                )
